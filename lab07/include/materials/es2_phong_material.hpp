    ...
    public:
        ES2PhongMaterial()
        {
            ...
            std::vector<std::string> uniforms{
                ...
                "texturing_mode1",
                "texture1_normal_sampler",
                "texture1_normal_enabled",
        ...
        void update(std::shared_ptr<Scene> scene, std::shared_ptr<Mesh> mesh) final
        {
            ...
            if (_texture1) {
                ...
                if (_texture1->is_enabled()) {
                    ...
                    _texture1_normal->update(1);
                    int texture1_normal_enabled_uniform_location{_shader->get_uniforms().at("texture1_normal_enabled")};
                    glUniform1i(
                        texture1_normal_enabled_uniform_location,
                        (GLint) _texture1_normal->is_enabled()
                    );

                    if (_texture1_normal->is_enabled()) {
                        int texture1_normal_sampler_uniform_location{_shader->get_uniforms().at("texture1_normal_sampler")};
                        glUniform1i(texture1_normal_sampler_uniform_location, 1);
                    }
                }
            }
            ...
            if (_texture2) {
                ...
                if (_texture2->is_enabled()) {
                    ...
                    glUniform1i(texture2_sampler_uniform_location, 2);
