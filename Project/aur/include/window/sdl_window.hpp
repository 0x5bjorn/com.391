...
namespace aur
{
    ...
    public:
        ...
        ) : Window{name, width, height}
        {
            ...
            _on_exit = [&]() { exit(0); };
            _on_key_down = [&](int key, const Uint8* state) { if (key == SDLK_ESCAPE) { exit(0); }};
            _on_mouse_move = [&](int x, int y, int x_rel, int y_rel) { };
            _on_mouse_down = [&](int button, int state, int x, int y) { };
        }
        ...
        void poll() final
        {
            ...
            while (SDL_PollEvent(&event)) {
                if (event.type == SDL_QUIT) {
                    ...
                    _on_key_down(event.key.keysym.sym, _state);
                } else if (event.type == SDL_MOUSEMOTION) {
                    _on_mouse_move(event.motion.x, event.motion.y, event.motion.xrel, event.motion.yrel);
                } else if (event.button.state == SDL_PRESSED || event.button.state == SDL_RELEASED) {
                    _on_mouse_down(event.button.button, event.button.state, event.button.x, event.button.y);
                }
